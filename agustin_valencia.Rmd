...
The printout below (close to zero values have been omitted) confirms that, after sorting their contributions, just the two first PCs represent 99.64% of the total variance. So the dimensionality could be reduced from 127 dimensions to just two.
...
ggplot() + geom_point( aes(x=res$x[,2], y=res$x[,1]) ) +
...
PC1 is explained mainly by the last variable in the original space which refers to viscosity measures, on the other hand PC2 explains a linear combination of the other variables.
...
$W$ matrix contains the unmixing coefficients. It can be seen that the traceplots from it are somewhat similar to the ones from PCA but in a mirrored fashion. This will allow to \emph{undo} the compression done in PCA and get again the original variables.
...
# 4.3.b - Scores
ggplot() + geom_point( aes(x=ica$S[,2], y=ica$S[,1]) ) +
